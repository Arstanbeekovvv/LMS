...
--DDL
--add column and update constraint
...
alter table owners add constraint emails unique (email);
alter table customers rename column family_status to marital_status;
alter type family_status rename to marital_status;
alter type marital_status rename to family_status;
alter type house_type rename to hsh;
alter type hh rename to house_type;
alter type house_type rename value 'HOUSE' to 'House';
alter type house_type rename value 'APARTMENT' to 'Apartment';
alter type genderss rename to gender;
...
alter table owners alter column gender type genders using gender::genders;
-- insert
...
-- 1. Owner_лердин аттарынын арасынан эн коп символ камтыган owner_ди жана анын уйун(House) чыгар.
    select max(length(owners.first_name)), h.house_type from owners
    inner join houses h on owners.id = h.owner_id group by house_type;
-- 2. Уйлордун баалары 1500, 2000 дин аралыгында бар болсо true чыгар, жок болсо false чыгар.
    select description, house_type, price, true or false from houses where price between '1500' and '2000';
-- 3. id_лери 5, 6, 7, 8, 9, 10 го барабар болгон адресстерди жана ал адрессте кайсы уйлор бар экенин чыгар.
    select a.*, h.* from addresses a
...
-- 4. Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.
    select h.*, o.first_name, c.first_name, a.name from houses h
        inner join rent_info ri on h.id = ri.house_id
        inner join agencies a on ri.agency_id = a.id
        inner join customers c on ri.customer_id = c.id
        inner join owners o on h.owner_id = o.id;
-- 5. Клиенттердин 10-катарынан баштап 1999-жылдан кийин туулган 15 клиентти чыгар.
    select * from customers c where extract(year from c.date_of_birth) > 1999   offset 10 limit 15;
-- 6. Рейтинги боюнча уйлорду сорттоп, уйлордун тайптарын, рейтингин жана уйдун ээлерин чыгар. (asc and desc)
    select h.house_type, h.rating, o.first_name, o.last_name from houses h inner join owners o on o.id = h.owner_id order by h.rating;
-- 7. Уйлордун арасынан квартиралардын (apartment) санын жана алардын баасынын суммасын чыгар.
    select count('Apartment') as apartments, sum(price) from houses where house_type in ('Apartment');
-- 8. Агентсволардын арасынан аты ‘My_House’ болгон агентсвоны, агентсвонын адресин жана анын бардык уйлорун, уйлордун        адрессин чыгар.
    select a.*, h.* from agencies ag
               inner join addresses a on a.id = ag.address_id
                    inner join houses h on ag.address_id = h.address_id
               where ag.name = 'My House';
...
-- 12.Уйлордун арасынан рейтингтери чон, кичине, орточо болгон 3 уйду чыгар.
    select house_type, max(houses.rating), house_type,  avg(houses.rating), house_type,  min(houses.rating) from houses group by house_type, house_type, house_type;
-- 13.Уйлору жок киленттерди, клиенттери жок уйлорду чыгар.
    select c.*, h.* from customers c full join rent_info ri on c.id = ri.customer_id full join houses h on h.id != ri.house_id ;
...
-- 15.‘A’ тамга менен башталган уйдун ээсинин аттарын, клиенттердин аттарын чыгар.
    select owners.first_name, c.first_name from owners inner join rent_info ri on owners.id = ri.owner_id inner join customers c on c.id = ri.customer_id where owners.first_name like 'A%';
